...
/**
 * Vychází z CXF {@link org.apache.cxf.interceptor.AbstractLoggingInterceptor}u, pouze používá náš
 * vlastní {@link PrettyPrintXMLStreamWriter} místo
 * {@link org.apache.cxf.staxutils.PrettyPrintXMLStreamWriter}u.
 * <p>
 * 
...
public abstract class AbstractLoggingInterceptor extends AbstractPhaseInterceptor<Message> {
    ...
    protected void writePayload( StringBuilder builder, CachedOutputStream cos, String encoding, String contentType ) throws Exception {
        ...
        if ( isPrettyLogging() && ( contentType != null && contentType.indexOf( "xml" ) >= 0 && contentType.toLowerCase().indexOf( "multipart/related" ) < 0 ) && cos.size() > 0 ) {
            ...
            xwriter = createPrettyPrintWriter( xwriter );
            InputStream in = cos.getInputStream();
            try {
                Reader reader = new InputStreamReader( in, encoding );
                StreamSource source = new StreamSource( reader );
                StaxUtils.copy( source, xwriter );
            }
            catch ( XMLStreamException xse ) {
                // ignore
                // pokud se XML loguje nespravne, vyplati se chyby si prohlidnout!
            }
            finally {
                try {
                    xwriter.flush();
    ...
    /**
     * Transform the string before display. The implementation in this class does nothing. Override
     * this method if you wish to change the contents of the logged message before it is delivered
     * to the output. For example, you can use this to mask out sensitive information.
     * 
    ...
    private PrettyPrintXMLStreamWriter createPrettyPrintWriter( XMLStreamWriter xwriter ) {
        PrettyPrintXMLStreamWriter writer = new PrettyPrintXMLStreamWriter( xwriter, 2 );
        writer.setShrinkLongCharacters( !isDebugEnabled() );
        return writer;
    }

    private boolean isDebugEnabled() {
        org.slf4j.Logger logger = LoggerFactory.getLogger( "org.apache.cxf.services" );
        return logger.isDebugEnabled() || logger.isTraceEnabled();
    }

	public boolean isWritePayload() {
		return writePayload;
	}

	public void setWritePayload(boolean writePayload) {
